...
<script type="text/javascript">
	...
	var eventData =
	<?php if ($eventData) echo json_encode($eventData).';'; else echo 'null;'; ?>

	var eventHash = {};
	var milestones =
	<?php if ($milestones) echo json_encode($milestones).';'; else echo 'null;'; ?>
...
<div id="light" class="white_content">
	...
	<div class="con">
		...
		<input type="button" class="btn btn-success" id="notNow" onclick="javascript:window.location.href='/dashboard'"
		       value="not now">
...
<script>
	...
	if (getUrlParam("progress") == 1) {
		show('light');
	} else if (getUrlParam("progress") == 2) {//settings??sync ??facebook??
		show('light');
		$('#submitImport').hide();
		$('#notNow').hide();
		$('#askConfirm').hide();
		...
		<fieldset>
			<table>
				<tr>
					...
					<td><input type="text" name="" description"" id="description" class="text ui-widget-content
						ui-corner-all">
					</td>
				</tr>
				<tr>
					<td><label for="quantity">Quantity</label></td>
					<td><input type="text" name="quantity" id="quantity" class="text ui-widget-content ui-corner-all">
					</td>
				</tr>
			</table>
...
jQuery(document).ready(function ($) {
	...
	//Render events on the calendar for month view
	var toRender = <?php echo json_encode($data);?>;
	var myEvents = getViewDate(toRender, 'month');
	//Render events for week view and day view
	var toRenderForWeek = <?php echo json_encode($dataForWeek);?>;
	var myEventsForWeek = getViewDate(toRenderForWeek, 'week');

	function getViewDate(viewData, view) {
		var toRender = viewData,
			...
			for (var j = 0; j < toRender[i].length; j++) {
				if (toRender[i][j].event_date) {
					...
					if (view == 'week') {
						...
						var notesFrom = toRender[i][j].notesFrom;
					}
					var obj = {
	...
	calendar.fullCalendar({
		...
		viewRender: function (view, element) {
			...
			if (view.name == 'agendaWeek' || view.name == 'agendaDay') {
				...
				calendar.fullCalendar('removeEventSource', myEventsForWeek);
				calendar.fullCalendar('addEventSource', myEventsForWeek);

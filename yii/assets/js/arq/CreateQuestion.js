...
var CreateQuestion = {
		...
		createQuestionTypesSelect:function(types) {
			...
			var html = '<select class="question_category" name="selected_question_type">',
				self = this;
			
			html+='<option class="question_category" disabled selected value="">Select a question type to ask</option>';
			for (var i = 0;i<types.length;i++) {
				this.typeHash[types[i].name] = types[i].question_type_id;
				html += '<option class="question_category" id="TYPE_'+types[i].question_type_id+'">'+types[i].name+'</option>';
			}
			html += '</select>';
		...
		basicTemplate:[
		               '<form id="createQuestion">',
		               		'<div class="arq-form tile-content">',
								'<h3>What is your question?</h3>',
								'<div  style="margin-left:15px;float:right;">',
									'<select class="question_category" name="question_category_id">{QUESTION_CATEGORIES}</select>',
								'</div>',								
		...
		MCAddon:[
				'<div class="tile-content">',
					...
					'<br>',
					'<input type="text" class="mc form-control" id="field-1" placeholder="Choice 1" name="choice_1">',
					'<br>',
					'<input type="text" class="mc form-control" id="field-2" placeholder="Choice 2" name="choice_2">',
					'<br>',
					'<input type="text" class="mc form-control" id="field-3" placeholder="Choice 3" name="choice_3">',
					'<br>',
					'<input type="text" class="mc form-control" id="field-4" placeholder="Choice 4" name="choice_4">',
				'</div>'].join(""),

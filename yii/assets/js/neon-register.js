...
function _handleRegistrationError(response)
{
	...
	setTimeout(function()
	{
		...
		{
			$('.form-login-error h3').html(response.error);
		} else
		{
			$('.form-login-error h3').html('Registration failed; please try again');
		}
		...
		setTimeout(function() {
			...
			neonRegister.$steps.slideDown('normal', function()
			{
			...
			submitHandler: function(ev)
			{
				...
				neonRegister.setPercentage(30, function()
				{
					...
					neonRegister.setPercentage(98, function()
					{
						...
						// radio buttons
						$('input:radio:checked').each(function(item,i) {
						...
						$.ajax({
							...
							error: function()
							{
								...
								if (_handleRegistrationError) _handleRegistrationError();
								return;
							},
							success: function(response)
							{
								...
								*/
								...
								if (!response || !response.success || response.success !=1)
								{
									...
									if (_handleRegistrationError) _handleRegistrationError(response);
									return;
								}
								...
								setTimeout(function()
								{
									...
									neonRegister.$steps.slideUp('normal', function()
									{
										...
										if (response.redirect) setTimeout(function() {window.location.href=response.redirect;},2000);
									});
